...
func (g *proxy) generateNoProxyDialer() {
	...
	g.gen.P("u, err := url.Parse(addr)")
	g.gen.P("if err != nil {")
	g.gen.P("return nil, err")
	g.gen.P("}")
	g.gen.P("log.Printf(\"dialing %s://%s\", u.Scheme, u.Hostname())")
	g.gen.P("return net.Dial(u.Scheme, u.Hostname())")
	g.gen.P("}")
	/*
		g.gen.P("switch u.Scheme {")
		g.gen.P("case \"unix\":")
		...
		g.gen.P("default:")
		g.gen.P("dst, err := net.ResolveTCPAddr(\"tcp\", addr)")
		g.gen.P("if err != nil {")
		g.gen.P("  return nil, err")
		g.gen.P("}")
		g.gen.P("return net.DialTCP(\"tcp\", nil, dst)")
		g.gen.P("}")
		g.gen.P("return nil, nil")
		g.gen.P("}")
	*/
}
