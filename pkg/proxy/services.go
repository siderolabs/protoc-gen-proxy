...
func (g *proxy) generateClientFns(serviceName, pkgName string) {
	...
	g.P(g.Clients, "conn, err := grpc.Dial(fmt.Sprintf(\"%s:%d\", target, 50000),")
	g.P(g.Clients, "grpc.WithTransportCredentials(creds),")
	g.P(g.Clients, "grpc.WithContextDialer(noProxyDialer),")
	g.P(g.Clients, ")")
